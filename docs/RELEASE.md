...
### 2026-09-09 조작·부스트·미니맵 후속

- 일반 주행 상한을 `12u/s`(`96km/h`)로 조정하고 W+Shift 부스트가 `34u/s`(`272km/h`)에 도달하도록 가속 응답을 개선
- 부스트 해제 즉시 일반 상한으로 복귀하며 제동·충돌 밀어내기 회귀를 유지
- 조작법 안내와 업데이트 내역을 별도 버튼으로 분리하고 모달 ESC·포커스·일시정지 복귀 및 모바일 내부 스크롤을 검증
- 미니맵에 NPC 8대·경찰 2대·플레이어 1대의 heading 방향 삼각형을 표시하고 플레이어를 마지막에 크게 렌더링
